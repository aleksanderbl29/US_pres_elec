...
polls<-read_csv("poll_list.csv") #Recall: you can update polls from 538
...
state_abb_list <- read.csv("potus_results_76_20.csv") %>%
  pull(state) %>% unique()
...
###############################################################
library("readxl")
states2020 <- read_excel("2020.xlsx") %>%
...
state_data <- read_csv("potus_results_76_20.csv")
...
#Census data#
census <- read_csv("acs_2023_variables.csv")
census <- census %>%
  filter(!is.na(state)) %>%
...
white_evangel_pct <- read_csv("white_evangel_pct.csv") %>%
  gather(variable,
         value,
         2:(ncol(.)))
...
# run TFC model
prior_model <- lm(
  formula = incvote ~  juneapp + q2gdp, # model equation
...
national_mu_prior <- predict(object = prior_model, # your TFC model
                             newdata = tibble(q2gdp = 2.8, # these values are for 2024 predictions
                                              juneapp = 0))
...
### Run the model ###
###############################################################
out <- stan(file = "poll_model_2024_2.stan", # this is stan model declared above
...
saveRDS(out, file = "new_output1.rds")
